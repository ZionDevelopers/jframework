...
class image {
	...
	public function newSize($w, $h, $forceScale = false) {
		...
		$this->_newImage ();
		$this->_import ( $this->imgName );
		$this->_newSize ();
	
	}
	
	protected function _needScale() {
	 ...
	 * @access private
	 */
	protected function _setNoScale() {
	 ...
	 * @access private
	 */
	protected function _forceScale() {
	 ...
	 * @return resource
	 */
	protected function _import($imgName) {
		try {
	...
	public function text($text, $size = 12, $x = 0, $y = 0, array $color = array(0,0,0), $font = 'arial') {
		...
		imagettftext ( $this->newImgRc, $size, 0, $x, $y, $color, APP_DIR . '/library/fonts/' . $font . '.ttf', $text );
	}
